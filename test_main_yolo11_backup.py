...
# Backup for YOLO11/320x320 kmodel packages.
# Change this path when using a different YOLO11 kmodel file.
...
def parse_flat_detection(det):
    ...
    x2 = float(det[4])
    y2 = float(det[5])
    if cls_id in LABELS and valid_box(x1, y1, x2, y2):
...
def main():
    print("K230 YOLO11 backup model start")
    ...
    try:
        while True:
            ...
            yolo.draw_result(res, pl.osd_img)  # Keep the original LCD detection overlay.
